...
1. **Tampermonkey 브릿지 스크립트** (`janitor-bridge_user.js`) — **필수.** janitorai.com 페이지에서 버튼 한 번으로 캐릭터/로어북을 SillyTavern으로 전송합니다.
2. **백엔드 플러그인** (`backend_plugin/janitor.js`) — **필수.** 브릿지 스크립트가 보낸 데이터를 실제로 캐릭터 카드(PNG)나 로어북(World Info)으로 저장하는 서버 쪽 코드입니다.
3. **SillyTavern 확장** (`index.js`, `manifest.json`, `style.css`) — 설치 안내를 보여주는 패널입니다. 필수는 아니지만 설치해두면 좋습니다.

---

## 1. Tampermonkey 브릿지 스크립트 설치 (필수, 가장 먼저)
...
## 3. SillyTavern 확장 설치 (선택, 안내용 패널)

기능은 없고 "브릿지 스크립트를 설치하세요"라는 안내 패널만 추가됩니다. 원하면 설치하세요.
...
### Tampermonkey를 폰에서 쓸 수 있나요?
가능하지만 브라우저를 잘 골라야 합니다.
- **Kiwi 브라우저는 비추천**: 2026년 1월부로 개발이 완전히 중단됐고, Tampermonkey 공식 FAQ에서도 유지보수 안 되는 브라우저라 다른 걸 쓰라고 안내하고 있습니다.
- **추천 조합 (Android)**: **Firefox for Android** 설치 → Firefox 안에서 **Tampermonkey 애드온** 설치. Tampermonkey가 공식적으로 지원하는 조합이라 가장 안정적입니다. (Microsoft Edge for Android도 Tampermonkey가 내장된 버전을 지원합니다.)
- **iOS(아이폰)**: 앱스토어의 **Tampermonkey** 공식 앱을 설치해서 Safari 확장으로 등록하면 됩니다.

### 설치 명령어

**1) SillyTavern이 아직 없다면 설치**
```bash
...
./start.sh
```
처음 실행되면 필요한 패키지를 자동으로 설치하고 서버가 켜집니다. `Ctrl+C`로 종료하고 아래 단계를 진행하세요.
