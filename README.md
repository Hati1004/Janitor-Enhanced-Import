...
2. **webp→PNG 변환용 패키지 설치** — JanitorAI 아바타는 대부분 webp 포맷이라, 이 패키지가 없으면 캐릭터 임포트 시 에러가 납니다. `plugins` 폴더 안에 있는 `node_modules`를 참조하므로, `plugins` 폴더에서 설치합니다.
   ```bash
...
- **로어북 엔트리를 못 찾음**: 로어북 소스 네모 창 끝까지 보이는지 확인하세요.
