...
1. `backend_plugin/janitor.js`를 SillyTavern 서버의 `plugins` 폴더에 **파일 하나로 그대로** 복사합니다. (폴더로 감쌀 필요 없이, 파일명도 `janitor.js` 그대로 두면 됩니다.)
   ```
   SillyTavern/plugins/janitor.js
   ```
   (`plugins` 폴더가 없으면 SillyTavern 루트에 새로 만들어 주세요.)
2. **webp→PNG 변환용 패키지 설치 (필수)** — JanitorAI 아바타는 대부분 webp 포맷이라, 이 패키지가 없으면 캐릭터 임포트 시 에러가 납니다. `plugins` 폴더 안에 있는 `node_modules`를 참조하므로, `plugins` 폴더에서 설치합니다.
   ```bash
   cd SillyTavern/plugins
   npm install sharp
   ```
3. SillyTavern의 `config.yaml`에서 서버 플러그인이 활성화되어 있는지 확인합니다.
   ```yaml
   enableServerPlugins: true
   ```
4. SillyTavern 서버를 **재시작**합니다. 콘솔에 플러그인 로드 로그가 뜨면 정상입니다.
