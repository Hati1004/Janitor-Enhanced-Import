...
window.runJaiConvert = async function(mode) {
    ...
    const cookie = document.getElementById('jai_cookie_input')?.value?.trim(); // 쿠키 값 가져오기
    ...
    if (mode === 'download') {
        ...
        try {
            data = await $.ajax({
                ...
                data: JSON.stringify({ url, cookie }), contentType: 'application/json'
            });
        } catch (err) {
    ...
    if (!data?.success) return setStatus(`❌ ${data?.error || '저장 실패'}`, 'error');
...
window.toggleJaiDrawer = function() {
    ...
    i.className = h ? 'inline-drawer-icon fa-solid fa-circle-chevron-up up' : 'inline-drawer-icon fa-solid fa-circle-chevron-down down';
};
    ...
    <div class="inline-drawer" id="jai_panel_container">
        ...
        <div class="inline-drawer-content" id="jai_content_wrap" style="display:none; padding:10px;">
            ...
            <input type="password" id="jai_cookie_input" class="text_pole"
                   placeholder="[선택] 웹 브라우저 쿠키 (401 에러/로어북 누락 방지용)"
