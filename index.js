...
// ── JanitorAI Bearer 토큰: 브라우저에만 저장, 서버로는 요청할 때만 전송 ──
const JAI_TOKEN_KEY = 'jai_janitor_token';
function getSavedToken() {
    try { return localStorage.getItem(JAI_TOKEN_KEY) || ''; } catch (e) { return ''; }
}
function saveToken(token) {
    try { localStorage.setItem(JAI_TOKEN_KEY, token || ''); } catch (e) { /* 무시 */ }
}
...
// ── 네이티브 API를 "실제 브라우저"에서 직접 호출 ─────────────────
// 서버(Node)에서 호출하면 Cloudflare 봇 감지에 걸려 403이 나므로,
// 반드시 이 함수처럼 브라우저 JS에서 fetch해야 한다.
function extractUuid(url) {
    const m = url.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i);
    return m ? m[0] : null;
}

async function fetchNativeApiData(url) {
    const token = getSavedToken();
    if (!token) return null; // 토큰 없으면 그냥 백엔드의 datacat/JannyAI 폴백에 맡김
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`[Janitor] 네이티브 API 브라우저 fetch 실패: HTTP ${res.status} (토큰 만료/오류 가능성)`);
            return null;
        }
        ...
        console.warn('[Janitor] 네이티브 API 브라우저 fetch 실패:', e.message);
        return null; // 네트워크 에러 등 → 백엔드 폴백으로
    }
}
...
window.runJaiConvert = async function(mode) {
    ...
    setStatus('🔄 캐릭터 데이터 확인 중...', 'info');
    const nativeApiData = await fetchNativeApiData(url);
    if (getSavedToken() && !nativeApiData) {
        setStatus('⚠️ 저장된 토큰으로 직접 가져오기 실패 (만료됐을 수 있음). datacat/JannyAI로 시도합니다...', 'info');
    }
    ...
    if (mode === 'download') {
        ...
        try {
            data = await $.ajax({
                ...
                data: JSON.stringify({ url, nativeApiData }), contentType: 'application/json'
            });
        } catch (err) {
...
function initUI() {
    ...
    <div class="inline-drawer" id="jai_panel_container">
        ...
        <div class="inline-drawer-content" id="jai_content_wrap" style="display:none; padding:10px;">
            ...
            <details style="margin-bottom:10px;">
                <summary style="cursor:pointer; font-size:0.85em; opacity:0.85;">🔑 JanitorAI 토큰 설정 (datacat/JannyAI 없이 안정적으로 받아오려면 필요)</summary>
                <p style="font-size:0.78em; opacity:0.7; margin:6px 0;">
                    janitorai.com에 로그인한 상태로 아무 캐릭터 페이지에서 F12 → Network → Fetch/XHR 탭에서
                    <code>hampter/characters/...</code> 요청의 Request Headers 중 <code>Authorization: Bearer ...</code> 값을
                    복사해서 붙여넣으세요. 이 값은 브라우저에만 저장되며, 서버로는 카드를 받아올 때만 전송됩니다.
                    <b>비밀번호처럼 다른 사람과 공유하지 마세요.</b>
                </p>
                <input type="password" id="jai_token_input" class="text_pole"
                       placeholder="Bearer 뒤에 오는 토큰 값만 붙여넣기"
                       style="width:100%; box-sizing:border-box; margin-bottom:6px;">
                <button class="menu_button menu_button_icon" style="width:100%;" onclick="window.saveJaiToken()">💾 토큰 저장</button>
                <div id="jai_token_status" style="font-size:0.78em; margin-top:6px;"></div>
            </details>
    ...
    const tokenInput = document.getElementById('jai_token_input');
    const tokenStatus = document.getElementById('jai_token_status');
    if (tokenInput) {
        const existing = getSavedToken();
        if (existing) {
            tokenInput.value = existing;
            if (tokenStatus) tokenStatus.textContent = '✅ 저장된 토큰이 있습니다.';
        }
    }
}

window.saveJaiToken = function() {
    const val = document.getElementById('jai_token_input')?.value?.trim();
    saveToken(val);
    const tokenStatus = document.getElementById('jai_token_status');
    if (tokenStatus) tokenStatus.textContent = val ? '✅ 토큰 저장 완료.' : 'ℹ️ 토큰을 비웠습니다 (datacat/JannyAI만 사용).';
};
